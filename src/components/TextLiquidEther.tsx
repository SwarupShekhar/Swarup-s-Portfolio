...
interface TextLiquidEtherProps {
  mouseForce?: number;
  cursorSize?: number;
  isViscous?: boolean;
  viscous?: number;
...
export default function TextLiquidEther({
  mouseForce = 20,
  cursorSize = 100,
  isViscous = false,
  viscous = 30,
...
}: TextLiquidEtherProps): React.ReactElement {
  const mountRef = useRef<HTMLDivElement | null>(null);
  const webglRef = useRef<any | null>(null);
  ...
  useEffect(() => {
    ...
    class Advection extends ShaderPass {
      ...
      updateWithParams(params: { dt: number; isBounce: boolean; BFECC: boolean }) {
    ...
    class Viscous extends ShaderPass {
      ...
      updateWithParams({ viscous, iterations, dt }: { viscous: number; iterations: number; dt: number }) {
        let fbo_in, fbo_out;
    ...
    class Poisson extends ShaderPass {
      ...
      updateWithParams({ iterations }: { iterations: number }) {
        let p_in, p_out;
        for (let i = 0; i < iterations; i++) {
          if (i % 2 === 0) {
    ...
    class Pressure extends ShaderPass {
      ...
      updateWithParams({ vel, pressure }: { vel: any; pressure: any }) {
        this.uniforms!.velocity.value = vel.texture;
    ...
    class Simulation {
      ...
      private getFloatType(): THREE.TextureDataType {
      ...
      update() {
        ...
        this.advection.updateWithParams({
          dt: this.options.dt,
          isBounce: this.options.isBounce,
          BFECC: this.options.BFECC
        });
        this.externalForce.updateWithParams({
          cursor_size: this.options.cursor_size,
        ...
        if (this.options.isViscous) {
          vel = this.viscous.updateWithParams({
            viscous: this.options.viscous,
            iterations: this.options.iterations_viscous,
            dt: this.options.dt
          })!;
        }
        this.divergence.updateWithParams({ vel });
        const pressure = this.poisson.updateWithParams({
          iterations: this.options.iterations_poisson
        });
        this.pressure.updateWithParams({ vel, pressure });
      }
    }
    ...
    class WebGLManager {
      props: { $wrapper: HTMLElement; autoIntensity: number; takeoverDuration: number };
      output!: Output;
  ...
  return <div ref={mountRef} className={`text-liquid-ether-container ${className || ''}`} style={style} />;
}